...
- `Packages/HagtampKit` — Swift package with the testable core:
  ...
  - `Milkdrop` — MilkDrop presets without the GPU: `MilkdropPreset` (.milk), `EELProgram` (the expression language compiled to closures), `MilkdropEngine` (one preset's frame: warp mesh, waves, shapes, borders, composite), `MilkdropSession` (switches with blending, sound analysis). `MilkdropMetal` — `MilkdropRenderer`, the Metal passes (shader source compiled at run time).
