...
App state lives in `Storage`: UserDefaults, `~/Library/Application Support/Hagtamp` (playlist, library index, local favourites and playlists in `Playlists/`, login token, skins, music kept offline in `Offline/`) and `~/Library/Caches/Hagtamp` (audio cache, covers, server responses: all fetchable again). The self test swaps all three for its own so it never touches the user's state.
