...
- `Packages/HagtampKit` — Swift package with the testable core:
  ...
  - `PlayerCore` — player domain without dependencies: `TrackInfo`, `Playlist` (order, Winamp selection, editing), `PlaylistFile` (m3u/m3u8/pls), `Lyrics` (LRC and plain), equalizer presets, `.eqf` files, loudness normalization (`Loudness`: ReplayGain values and the gain a track gets; `LoudnessIndex`: tags and measurements by track, album gains combined from measured tracks).
  ...
  - `LibraryKit` — local library: `LocalLibrary` (actor: scans folders, rereads only changed files, JSON index), `LibraryCatalog` (artists / albums / tracks grouping, search), `LibraryEntry` (tags of one file), `LibraryFavourites` (local favourites: catalog ids and paths), `FolderWatcher` (FSEvents).
  - `StreamingInput` — Objective-C: `StreamingInputSource` (SFB input source reading a file that is still downloading; unseekable until complete), `LiveInputSource` (internet radio, fed from memory), `AudioStreamDecoder` (AAC streams via AudioFileStream + AudioConverter), `PositionedDecoder` (starts a decoder part way through, opened on the player's thread) and `GainDecoder` (a track's normalization gain, applied as it decodes, so gapless handovers change it on the exact sample).
  - `AudioCore` — playback on SFBAudioEngine: `AudioEngine` (gapless queue of files, `StreamingTrack`s and `LiveStream`s, EQ, balance, volume, per-track gain, visualizer sample tap), `LiveStream` (radio: HTTP, ICY titles, .pls/.m3u), `TrackInfo.read` (tags/properties), `LoudnessMeter` (ITU BS.1770 integrated loudness) and `LoudnessReading` (a file's ReplayGain tags, else measured).
  - `Milkdrop` — MilkDrop presets without the GPU: `MilkdropPreset` (.milk), `EELProgram` (the expression language compiled to closures), `MilkdropEngine` (one preset's frame: warp mesh, waves, shapes, borders, composite), `MilkdropSession` (switches with blending, sound analysis). `MilkdropMetal` — `MilkdropRenderer`, the Metal passes (shader source compiled at run time). Both build with `-O` even in debug (preset code runs thousands of times a frame); the app draws them on a render thread paced by `CAMetalDisplayLink` (`VisualizationWindowController`); `PresetLibrary.swift` has the preset list (background, folder watched, slow presets) and the preset browser.
...
mpg123's per-decoder setup isn't thread-safe: open decoders only on SFB's decoding thread (the app does), and keep tests that open MP3 decoders serialized. Loudness measuring decodes MP3 with Core Audio for that reason.

App state lives in `Storage`: UserDefaults, `~/Library/Application Support/Hagtamp` (playlist, library index, loudness index, local favourites and playlists in `Playlists/`, login token, skins, music kept offline in `Offline/`) and `~/Library/Caches/Hagtamp` (audio cache, covers, server responses: all fetchable again). The self test swaps all three for its own so it never touches the user's state.
