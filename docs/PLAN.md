...
6. **Done** — polish: Winamp's main menu (options button, clutter bar "O", right click: files, windows, Skins, Options, Playback, Visualization) and a Controls menu in the menu bar; Winamp's keys (Z X C V B, L, J, R, S, arrows, keypad 1/3, Ctrl+L/J/T/D/A/P/Z/V, Shift+V, Alt+S) with Play URL, Jump to Time, Start of List, 10 tracks back/forward, Stop with Fadeout, Stop After Current; skin browser (Alt+S: installed skins drawn as they look, `Application Support/Hagtamp/Skins`, opened skins are installed there) and a Skins submenu; media keys, headphones, Control Center and the Now Playing widget (`NowPlaying`: title, artist, album, time, cover). MilkDrop-style visualization: see below.
...
- [x] Keep offline: right-click a Navidrome album or playlist; its songs download and move to `Application Support/Hagtamp/Offline` (`offline.json`, `AudioCache.keepOffline`), apart from the cache in `~/Library/Caches/Hagtamp` that macOS, the size limit and Clear Cache may empty. Unpinned songs move back into the cache. Pinned rows get a dot, pins resync at launch (playlists change), a pinned song plays in whatever quality it was saved.
...
- [x] Visualization window (⌃⇧K), stage 1 of MilkDrop-compatible presets (.milk), our own engine rather than projectM (OpenGL, LGPL): `Milkdrop` (no GPU: preset parser, NS-EEL/EEL2 compiler to closures, per-frame and per-vertex code, the warp mesh with zoom/zoomexp/rot/warp/stretch/translate, 8 main wave modes, 4 custom waves, 4 custom shapes with instances and textures, borders, motion vectors, video echo, gamma/brighten/darken/solarize/invert, sound analysis with bass/mid/treb and their "att" versions, blended preset switches) and `MilkdropMetal` (feedback textures, warp, drawing, composite; shaders compiled at run time). Presets: eight of our own in `App/Presets`, the user's in `Application Support/Hagtamp/Presets`; switched every 20 s (random order, can be locked); keys as in MilkDrop; full screen. Frames are drawn on their own thread paced by `CAMetalDisplayLink` and pause while the window is covered. The preset list (`PresetLibrary`) is read in the background and follows the folder (FSEvents); the browser (P) searches it. A preset whose frames keep missing the budget (12 ms on average, or five 100 ms stalls) is marked slow and skipped by the rotation; preset loops also stop at a 150 ms per-frame deadline. Stage 2: MilkDrop 2 pixel shaders (HLSL through glslang + SPIRV-Cross to Metal), blur and noise textures; stereo samples for the few effects that use them.
- [x] Text size (Preferences > Appearance, 100-200%, 150% by default): the playlist, library lists (sidebar, fixed columns and header included) and lyrics; rows grow with the text (`ClassicUI.TextSize`). 100% is Winamp's 9 pt in 13 px rows, which golden checks and the self test use.
- [x] Lyrics window (⌥Y): the current track's words in a generic skinned frame, playlist colours and font, wrapped and centred. From Navidrome (OpenSubsonic `getLyricsBySongId`, which lyrics plugins answer too; `getLyrics` for older servers; cached for offline use) or, for local files, an .lrc next to the file or the lyrics tag. Synced lyrics (LRC) highlight the current line and keep it in the middle (a manual scroll pauses that for 4 s); a click on a line plays from there. The dev library has a made-up .lrc for Alpha Tones' Tone 1.
- [x] Local library window: folders chosen in Preferences, scanned in the background (`LibraryKit`: tags read only for new or changed files, index kept in `library.json`), Audio (artists | albums | tracks, compilations without album artist tags become "Various Artists") and Recently Added views, search, Play/Enqueue. Folders are watched (FSEvents via `FolderWatcher`): changes rescan a few seconds later. Not yet: Most Played / Never Played (needs play counts).
